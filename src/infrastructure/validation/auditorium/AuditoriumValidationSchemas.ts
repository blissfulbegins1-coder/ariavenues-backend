...
export const updateAuditoriumSchema = yup.object().shape({
  name: yup
    .string()
    .required('Name is required')
    .trim()
  ...
  status: yup
    ...
    .oneOf(['draft', 'maintenance', 'active'], 'Invalid status value')
    .default('draft'),
}).test('total-images-must-be-6', 'Exactly 6 images are required in total (existing and new combined)', function (value) {
